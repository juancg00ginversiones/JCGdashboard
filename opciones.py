...
def find_levels(df, spot):
    ...
    total_loss = [(df['oi_call']*np.maximum(S-df['strike'],0)*CONTRACT_MULT +
                   df['oi_put'] *np.maximum(df['strike']-S,0)*CONTRACT_MULT).sum() for S in strikes]
    max_pain = float(strikes[np.argmin(total_loss)])

    return float(call_wall), float(put_wall), float(gamma_flip), float(max_pain)

def procesar_indice(cfg):
    symbol = cfg["symbol"]
    print(f"  → {symbol}...")
    try:
        tk   = yf.Ticker(symbol)
...
def main():
    print("\n" + "="*55)
    print("  ANÁLISIS DE OPCIONES — SPY / QQQ / IWM")
    print("="*55)
    ...
    for cfg in INDICES:
        res = procesar_indice(cfg)
        if res:
            output["indices"][cfg["symbol"]] = res

    with open("opciones_data.json","w") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)

    print(f"\n✅ Guardado: opciones_data.json")
    for sym, data in output["indices"].items():
        print(f"\n  {sym} @ ${data['spot']}")
        for label, exp in data["expiries"].items():
            r = "✅ POSITIVO" if exp["regime"]=="POSITIVO" else "⚠️ NEGATIVO"
            print(f"    [{label}] {exp['expiry']} ({exp['dte']}d)")
            print(f"      Call Wall:  ${exp['call_wall']}")
            print(f"      Put Wall:   ${exp['put_wall']}")
            print(f"      Gamma Flip: ${exp['gamma_flip']}  {r}")
            print(f"      Max Pain:   ${exp['max_pain']}")
